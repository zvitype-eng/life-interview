# Life Interview — agent entry point (Codex and other agents)
...
`main` is canonical. A hosted session may start on an auto-generated `claude/**` or equivalent session branch — that is a workspace, not a destination. Converge before the session ends: `git push origin <branch>:main` (fast-forward only; no force). Merged `claude/**` branches are auto-deleted by `.github/workflows/cleanup-merged-branches.yml`.
